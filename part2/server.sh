...
( mvn -q exec:java -Dexec.mainClass="${SV_MAIN}" -Dexec.args="2") &
write_pid "replica2" $!
sleep 1   # ensure server binds before FE connects
echo "Starting Replica3..."
( mvn -q exec:java -Dexec.mainClass="${SV_MAIN}" -Dexec.args="3") &
write_pid "replica3" $!
sleep 1   # ensure server binds before FE connects
...
#o Launches at least 3 replicas
#o Completes initialisation within ten seconds.
